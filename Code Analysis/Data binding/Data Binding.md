...
### - 3. 단위 테스트(unit test) -
데이터 바인딩으로 view 오브젝트와 logic이 분리되면서 객체 의존성이 사라지고 단위별로 테스팅이 가능해진다.
...
아이템 추가 메소드가 특정 오브젝트가 존재함을 전제로 동작하므로 객체 의존성이 존재하며 메소드이외의 환경 세팅이 필요하므로 이러한 부분의 세팅을 해야한다는 점에서 시간 및 비용 문제가 발생한다.  
